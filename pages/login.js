import CardHeader from '@mui/material/CardHeader';
import CardContent from '@mui/material/CardContent';
import List from '@mui/material/List';
import ListItemButton from '@mui/material/ListItemButton';
import ListItemIcon from '@mui/material/ListItemIcon';
import ListItemText from '@mui/material/ListItemText';
import IconButton from '@mui/material/IconButton';
import Card from '@mui/material/Card';
import GoogleIcon from '@mui/icons-material/Google';
import TwitterIcon from '@mui/icons-material/Twitter';
import FacebookIcon from '@mui/icons-material/Facebook';
import CloseIcon from '@mui/icons-material/Close';
import GitHubIcon from '@mui/icons-material/GitHub';
import LinkedInIcon from '@mui/icons-material/LinkedIn';
import LoginIcon from '@mui/icons-material/Login';
import { Container, useMediaQuery } from '@mui/material';

import { signIn } from 'next-auth/react';

import { useState } from 'react';
import BackdropSpinner from '../src/components/progress/BackdropSpinner';
import Head from 'next/head';
import MainLayout from '../src/layouts/MainLayout';
const PROVIDERS = [
  {
    id: 'google',
    name: 'Google',
  },
  {
    id: 'facebook',
    name: 'Facebook',
  },
  {
    id: 'twitter',
    name: 'Twitter',
  },
  {
    id: 'github',
    name: 'GitHub',
  },
  {
    id: 'linkedin',
    name: 'LinkedIn',
  },
];

const Login = () => {
  const [loading, setLoading] = useState(false);

  const matchedSMDown = useMediaQuery((theme) =>
    theme.breakpoints.down('sm')
  );

  const onListItemClick = (providerId) => () => {
    setLoading(true);
    signIn(providerId);
  };

  const renderIcon = (id) => {
    switch (id) {
      case 'google':
        return (
          <GoogleIcon fontSize="small" color="google" />
        );
      case 'facebook':
        return (
          <FacebookIcon fontSize="small" color="facebook" />
        );
      case 'twitter':
        return (
          <TwitterIcon fontSize="small" color="twitter" />
        );
      case 'github':
        return (
          <GitHubIcon fontSize="small" color="github" />
        );
      case 'linkedin':
        return (
          <LinkedInIcon fontSize="small" color="linkedin" />
        );
      default:
        return <LoginIcon />;
    }
  };

  return (
    <>
      <Head>
        <title key="title">Login - Voxydev</title>
        <meta
          name="description"
          key="description"
          content="Log in or Create an account with Google Account, Twitter, or Facebook"
        />
        <meta
          name="keywords"
          key="keywords"
          content="Log in, Sign up, Voxydev, MUI, Material UI, Nextjs, Material UI V5, Blog, VoxyDev"
        />
        /** Facebook share */
        <meta
          property="og:url"
          key="og:url"
          content={`${process.env.NEXT_PUBLIC_WEB_APP_URL}/contact`}
        />
        <meta
          property="og:type"
          key="og:type"
          content="article"
        />
        <meta
          property="og:title"
          key="og:title"
          content="Login - VoxyDev"
        />
        <meta
          property="og:description"
          key="og:description"
          content="Login Tien Hoang Vo (the admin of VoxyDev) for work"
        />
        <meta
          property="og:image"
          key="og:image"
          content={`${process.env.NEXT_PUBLIC_WEB_APP_URL}/images/contact_page.png`}
        />
        <meta
          property="og:image:secure_url"
          key="og:image:secure_url"
          content={`${process.env.NEXT_PUBLIC_WEB_APP_URL}/images/contact_page.png`}
        />
        <meta property="og:image:width" content="480" />
        <meta property="og:image:height" content="360" />
        <meta
          property="og:image:type"
          content="image/png"
        />
        <meta
          property="og:image:alt"
          content="Login - VoxyDev"
        />
      </Head>
      <Container maxWidth="sm">
        <Card elevation={0}>
          <CardHeader
            title="Login"
            sx={{
              borderBottom: 2,
              borderColor: 'divider',
              p: (theme) => theme.spacing(1, 2),
            }}
            titleTypographyProps={{
              sx: {
                fontSize: '1rem',
                fontWeight: 700,
              },
            }}
          />

          <CardContent>
            <List>
              {PROVIDERS.map((provider, index) => (
                <ListItemButton
                  dense
                  key={provider.id}
                  onClick={onListItemClick(provider.id)}
                  sx={{
                    mt: index > 0 && 2,
                    border: 2,
                    borderColor: `${provider.id}.main`,
                    borderRadius: '4px',
                  }}
                >
                  <ListItemIcon>
                    {renderIcon(provider.id)}
                  </ListItemIcon>
                  <ListItemText
                    primaryTypographyProps={{
                      fontWeight: 700,
                    }}
                    primary={`Continue with ${provider.name}`}
                  />
                </ListItemButton>
              ))}
            </List>
          </CardContent>
          <BackdropSpinner open={loading} />
        </Card>
      </Container>
    </>
  );
};

Login.getLayout = (page) => <MainLayout>{page}</MainLayout>;

export default Login;
